...
const typeChoice = (types) =>
  types.length === 1 ? types[0] : choice(...types);
const bodyItem = ($, rule) => choice(prec(1, seq($.indentation, rule)), rule);
const terminated = ($, ...rules) => seq(...rules, $._terminator);
const keywordStatement = ($, keyword, ...rules) =>
  terminated($, keyword, ...rules);
...
const bracketLabel = ($, labelRule) =>
  seq("[", field("text", optional(labelRule)), "]");
...
const diagramDefinitions = [
  ...
  {
    name: "c4",
    types: ["c4", "C4Component", "C4Container", "C4Context", "C4Deployment"],
    support: SUPPORT.baseline,
  },
  {
    name: "class",
  ...
  },
  {
    ...
    types: ["journey"],
    statements: ($) => [
      choice($.journey_section_statement, $.journey_task_statement),
    ],
  },
  {
    name: "kanban",
    types: ["kanban", "kanban-beta"],
    support: SUPPORT.baseline,
  },
  {
    name: "mindmap",
  ...
  {
    name: "xychart",
    types: ["xychart", "xychart-beta"],
    support: SUPPORT.baseline,
  },
  {
    name: "sankey",
    types: ["sankey", "sankey-beta"],
    support: SUPPORT.baseline,
  },
  {
    name: "ishikawa",
    types: ["ishikawa", "ishikawa-beta"],
    support: SUPPORT.baseline,
  },
  { name: "venn", types: ["venn", "venn-beta"], support: SUPPORT.baseline },
  {
    name: "railroad",
    types: [
      "railroad-beta",
  ...
  {
    name: "agentflow",
    types: ["agentflow-beta"],
    header: { direction: true },
    statements: ($) => [$.flow_statement, $.agent_flow_statement],
  },
  {
    name: "usecase",
    types: ["usecase-beta"],
    statements: ($) => [$.flow_statement, $.usecase_declaration],
  },
...
const baselineDiagramDefinitions = diagramDefinitions.filter(
  (definition) => diagramSupport(definition) === SUPPORT.baseline,
);
const diagramTypes = diagramDefinitions.flatMap(
  (definition) => definition.types,
);
const baselineDiagramTypes = baselineDiagramDefinitions.flatMap(
  (definition) => definition.types,
);

const diagramRuleName = (definition) => `${definition.name}_diagram`;
const diagramHeaderRuleName = (definition) =>
  `${definition.name}_diagram_header`;
  ...
  seq(
    ...
    ...(options.showData
      ? [
          optional("showData"),
          optional(
            seq("title", field("title", alias($._line_remainder, $.line_text))),
          ),
        ]
      : []),
    optional(":"),
...
const diagramItem = ($, definition) => {
  ...
  const bodyRules =
    definition.indentStatements === false
      ? statementRules
  ...
  return choice(
    ...
    ...bodyRules,
    ...(["cynefin", "wardley", "usecase", "agentflow"].includes(definition.name)
      ? [$.unknown_statement]
      : []),
    $._terminator,
  );
...
const diagramRules = Object.fromEntries(
  scopedDiagramDefinitions.flatMap((definition) => [
      ...
      ($) =>
        prec.right(
          seq(
            ...
            repeat($[diagramItemRuleName(definition)]),
          ),
        ),
    ],
    [
    ...
    [diagramItemRuleName(definition), ($) => diagramItem($, definition)],
  ]),
);
...
module.exports = grammar({
  ...
  rules: {
    ...
    diagram: ($) =>
      choice(
        ...scopedDiagramDefinitions.map(
          (definition) => $[diagramRuleName(definition)],
        ),
        $.baseline_diagram,
      ),
    ...
    frontmatter_line: ($) =>
      seq(optional($.indentation), $.frontmatter_content),
    ...
    block_open: (_) => token(prec(21, /block[ \t]*:[A-Za-z0-9_]+/)),
    ...
    baseline_diagram_type: (_) =>
      token(prec(20, typeChoice(baselineDiagramTypes))),

    direction: (_) =>
      choice("TB", "TD", "BT", "RL", "LR", "BR", "<", ">", "^", "v"),
    ...
    title_statement: ($) =>
      keywordStatement(
        ...
        "title",
        optional(":"),
        optional(field("text", alias($._line_remainder, $.line_text))),
      ),
    ...
    agent_flow_statement: ($) =>
      terminated(
        $,
        "flow",
        field("id", $._identifier),
        optional($.square_label),
        optional($.shape_data),
      ),
    ...
    usecase_declaration: ($) =>
      terminated(
        ...
        field(
          "keyword",
          choice("actor", "systemBoundary", "businessActor", "businessUsecase"),
        ),
        field("name", choice($._identifier, $.quoted_string)),
        optional($._line_remainder),
      ),
    ...
    sequence_control_statement: ($) =>
      terminated(
        $,
        field("keyword", $.sequence_control_keyword),
        optional($._line_remainder),
      ),

    sequence_control_keyword: (_) =>
      token(
        prec(
          5,
          choice(
    ...
    sequence_message_statement: ($) =>
      prec(
        ...
        seq(
          ...
          optional(
            seq(":", field("message", optional($.sequence_message_text))),
          ),
          $._terminator,
        ),
      ),
    ...
    sequence_participant_statement: ($) =>
      seq(
        ...
        optional(
          seq($.sequence_as_keyword, field("alias", $.sequence_message_text)),
        ),
        $._terminator,
      ),

    sequence_autonumber_statement: ($) =>
      terminated($, $.sequence_autonumber_keyword, optional($._line_remainder)),

    sequence_loop_statement: ($) =>
      terminated(
        $,
        $.sequence_loop_keyword,
        field("label", optional($.sequence_message_text)),
      ),
    ...
    sequence_note_statement: ($) =>
      seq(
        ...
        choice(
          seq(
            field("position", $.sequence_note_side_position),
            $.sequence_of_keyword,
          ),
    ...
    sequence_participant_keyword: (_) =>
      token(prec(5, choice("participant", "actor"))),

    sequence_as_keyword: (_) => token(prec(5, "as")),

    sequence_loop_keyword: (_) => token(prec(5, "loop")),

    sequence_note_keyword: (_) => token(prec(5, choice("Note", "note"))),
    ...
    sequence_actor_list: ($) =>
      seq($.sequence_actor, repeat(seq(",", $.sequence_actor))),

    sequence_actor: ($) => $._identifier,

    sequence_arrow: (_) =>
      choice("<<->>", "<<-->>", "->>", "-->>", "-x", "--x", "-)", "--)"),

    sequence_message_text: ($) =>
      repeat1(choice($.html_tag, $.sequence_message_text_fragment)),

    sequence_message_text_fragment: (_) =>
      token(prec(PREC.remainder, /[^<\n\r;]+|</)),

    state_declaration_statement: ($) =>
      keywordStatement($, "state", field("name", $._identifier)),

    state_transition_statement: ($) =>
      seq(
        field("source", choice($.state_marker, $._identifier)),
        field("arrow", $.state_arrow),
        field("target", choice($.state_marker, $._identifier)),
    ...
    state_transition_label: (_) =>
      token(prec(PREC.remainder, /[^ \t\f\n\r;][^\n\r;]*/)),

    flow_direction_statement: ($) =>
      keywordStatement($, "direction", field("direction", $.direction)),

    subgraph_statement: ($) =>
      seq(
        "subgraph",
        optional(field("id", $._identifier)),
        optional(
          field(
            "label",
            choice($.square_label, $.quoted_string, $._line_remainder),
          ),
        ),
        $._terminator,
      ),
    ...
    link_style_statement: ($) =>
      seq(
        ...
        optional(seq("interpolate", field("interpolation", $._identifier))),
        optional($.style_list),
        $._terminator,
      ),
      ...
      seq(
        ...
        optional(
          choice(
            $.href_action,
            $.call_action,
            field("callback", $._identifier),
            $.quoted_string,
          ),
        ),
        optional($.quoted_string),
        optional(
          field("link_target", choice("_self", "_blank", "_parent", "_top")),
        ),
        $._terminator,
      ),
    ...
    flow_edge_statement: ($) =>
      prec(
        ...
        seq(
          ...
          repeat1(
            seq(field("edge", $.flow_edge), field("target", $.flow_node)),
          ),
          $._terminator,
        ),
      ),
    ...
    flow_node_statement: ($) =>
      prec(
        PREC.flow_statement,
        seq(
          choice($.standalone_flow_node, field("id", $._identifier)),
          $._terminator,
        ),
      ),
    ...
    standalone_flow_node: ($) =>
      seq(
        field("id", $._identifier),
        choice(field("shape", $.flow_shape), $.shape_data, $.class_annotation),
        repeat(
          choice(
    ...
    stadium_label: ($) =>
      seq("([", field("text", optional($.label_text)), "])"),
    subroutine_label: ($) =>
      seq("[[", field("text", optional($.label_text)), "]]"),
    cylinder_label: ($) =>
      seq("[(", field("text", optional($.label_text)), ")]"),
    ...
    flow_edge: ($) =>
      choice(
        seq(
          optional($.edge_id),
          field("operator", $.arrow),
          optional($.edge_label),
        ),
        seq(
          optional($.edge_id),
          field("operator", $.arrow_start),
          optional($.edge_label),
          field("operator_end", $.arrow),
        ),
      ),

    edge_id: ($) => seq(field("id", $._identifier), "@"),
    ...
    class_annotation: ($) => seq(":::", field("name", $._identifier)),
    ...
    architecture_group_statement: ($) =>
      seq(
        ...
        optional(seq("in", field("parent", $._identifier))),
        $._terminator,
      ),
    ...
    architecture_arrow: (_) =>
      token(/:?[LRTB]?[ \t]*[<>]?--(?:[^-\n\r;]+-)?[<>]?[ \t]*[LRTB]?:?/),
    architecture_group_marker: (_) => token("{group}"),
    architecture_icon: (_) => token(/\([\w:-]+\)/),
    architecture_title: ($) =>
      seq("[", choice($.quoted_string, $.label_text), "]"),

    class_note_statement: ($) =>
      seq(
        "note",
        optional(seq("for", field("target", $._identifier))),
    ...
    class_note_label: ($) =>
      repeat1(choice($.html_tag, $.class_note_text_fragment)),

    class_note_text_fragment: (_) =>
      token(prec(PREC.remainder, /[^<"\n\r;]+|</)),
    ...
    class_relationship_operator: (_) =>
      token(/<\|--|<\|\.\.|\*--|o--|-->|<--|--|\.\./),

    class_member_statement: ($) =>
      seq(
        field("class", $._identifier),
        ":",
    ...
    class_block: ($) =>
      seq(
        ...
        $._terminator,
        repeat(
          choice($.class_block_member_statement, $.comment, $._terminator),
        ),
        "}",
        $._terminator,
      ),

    class_block_member_statement: ($) =>
      terminated($, field("member", $.class_member)),

    class_member: ($) => choice($.class_property, $.class_method),

    class_property: ($) =>
      seq(
        optional(field("visibility", $.class_visibility)),
        optional(field("type", $._identifier)),
        field("name", $._identifier),
      ),
    ...
    class_entity_statement: ($) =>
      terminated($, optional("class"), field("name", $._identifier)),

    cynefin_statement: ($) =>
      choice(
        seq(field("domain", $.cynefin_domain), $._terminator),
        seq(
          field("from", $.cynefin_domain),
          "-->",
          field("to", $.cynefin_domain),
          optional(seq(":", field("label", $.quoted_string))),
          $._terminator,
        ),
      ),

    cynefin_domain: (_) =>
      choice("complex", "complicated", "clear", "chaotic", "confusion"),
    ...
    er_relationship_statement: ($) =>
      choice(
        seq(
          field(
            "source",
            choice($._identifier, alias($.er_identifier, $.identifier)),
          ),
    ...
    er_identifier: (_) => token(/[A-Za-z_][A-Za-z0-9_]*(-[A-Za-z0-9_]+)+/),

    er_relationship_label: (_) =>
      choice("places", "contains", "owns", "has", "includes", "uses"),

    er_relationship_operator: (_) =>
      token(/(?:\|\||\|o|}\||}o)(?:--|\.\.)(?:\|\||o\||\|\{|o\{)/),

    er_relationship_text: (_) =>
      token(prec(PREC.remainder, /[^ \t\f\n\r;][^\n\r;]*/)),
    ...
    er_entity_block: ($) =>
      seq(
        ...
        $._terminator,
        repeat(
          choice(bodyItem($, $.er_entity_attribute_statement), $._terminator),
        ),
        optional($.indentation),
        "}",
        $._terminator,
    ...
    er_entity_alias: ($) =>
      seq("[", field("text", choice($.quoted_string, $.label_text)), "]"),
    ...
    er_attribute_key_list: ($) =>
      seq($.er_attribute_key, repeat(seq(",", $.er_attribute_key))),
    ...
    er_attribute_statement: ($) =>
      seq(
        ...
        repeat1(field("attribute", $._identifier)),
        $._terminator,
      ),

    event_modeling_statement: ($) =>
      lineRemainderStatement(
        $,
        choice(
          "entity",
          "data",
    ...
    gantt_section_statement: ($) =>
      keywordNameStatement($, "section", $.gantt_section_name),

    gantt_section_name: (_) =>
      token(prec(PREC.remainder, /[^ \t\f\n\r;][^\n\r;]*/)),
        ...
        seq(
          ...
          repeat1(
            choice(
              ...
              seq("after", field("dependency", $._identifier)),
              ",",
            ),
          ),
          $._terminator,
    ...
    gantt_task_label: ($) => repeat1($._identifier),
    ...
    git_graph_statement: ($) =>
      choice(
        seq(
          "commit",
          repeat(choice($.git_option, $.quoted_string)),
          $._terminator,
        ),
        seq(
          "branch",
        ...
        seq(
          "merge",
          field("branch", choice($._identifier, $.quoted_string)),
          repeat($.git_option),
          $._terminator,
        ),
        seq(
    ...
    git_option: ($) =>
      seq(
        ...
        ":",
        field("value", choice($.quoted_string, $._identifier, $.number)),
      ),

    git_option_name: (_) =>
      choice("id", "msg", "tag", "type", "order", "parent"),

    info_statement: ($) => terminated($, "showInfo"),

    journey_section_statement: ($) =>
      keywordNameStatement($, "section", $.journey_section_name),

    journey_section_name: (_) =>
      token(prec(PREC.remainder, /[^ \t\f\n\r;][^\n\r;]*/)),
    ...
    journey_task_statement: ($) =>
      seq(
        ...
        field("score", $.number),
        optional(":"),
        field("actor", $._identifier),
        repeat(seq(optional(","), field("actor", $._identifier))),
        $._terminator,
      ),

    journey_task_label: ($) => repeat1($._identifier),

    mindmap_statement: ($) =>
      choice($.mindmap_node_statement, $.mindmap_icon_statement),
    ...
    mindmap_circle_label: ($) =>
      seq("((", field("text", optional($.mindmap_label)), "))"),
    ...
    packet_statement: ($) =>
      terminated(
        ...
        field(
          "range",
          choice(
            seq($.number, optional(seq("-", $.number))),
            seq("+", $.number),
          ),
        ),
        ":",
    ...
    quadrant_point_label: ($) => repeat1($._identifier),

    quadrant_axis_text: (_) =>
      token(prec(PREC.remainder, /[^ \t\f\n\r;\[\],:-][^-\n\r;\[\],:]*/)),
    ...
    radar_statement: ($) =>
      choice(
        seq(
          "axis",
          $.radar_axis,
          repeat(seq(",", $.radar_axis)),
          $._terminator,
        ),
        seq(
          "curve",
          $.radar_curve,
          repeat(seq(",", $.radar_curve)),
          $._terminator,
        ),
    ...
    radar_axis: ($) =>
      seq(
        field("name", $._identifier),
        optional(field("label", $.square_label)),
      ),
    ...
    radar_option_name: (_) =>
      choice("showLegend", "ticks", "max", "min", "graticule"),

    requirement_statement: ($) =>
      choice($.requirement_block, $.requirement_relationship_statement),
    ...
    requirement_block: ($) =>
      seq(
        ...
        repeat(
          choice(
            bodyItem($, $.requirement_property_statement),
            $.comment,
            $._terminator,
          ),
        ),
    ...
    timeline_statement: ($) =>
      choice($.timeline_section_statement, $.timeline_event_statement),

    timeline_section_statement: ($) =>
      keywordNameStatement($, "section", $.timeline_section_name),

    timeline_section_name: (_) =>
      token(prec(PREC.remainder, /[^ \t\f\n\r;][^\n\r;]*/)),
      ...
      prec(
        ...
        seq(
          ...
          repeat(
            choice(
              $.class_annotation,
              $.tree_icon_annotation,
              $.tree_description_annotation,
            ),
          ),
          optional(
            seq(
    ...
    wardley_statement: ($) =>
      choice($.wardley_keyword_statement, $.wardley_edge_statement),
      ...
      prec(
        ...
        terminated(
          ...
          prec(
            PREC.unknown,
            repeat1(
              choice($._identifier, $.number, $.quoted_string, $.unknown_token),
            ),
          ),
        ),
      ),

    identifier_list: ($) => seq($._identifier, repeat(seq(",", $._identifier))),
    ...
    label_text_fragment: (_) =>
      token(prec(PREC.remainder, /[^<\]\)\}\|\n\r;]+|</)),

    // Preserve whole identifiers whose prefix is a diagram keyword.
    _identifier: ($) =>
      choice($.identifier, alias($.keyword_identifier, $.identifier)),
    keyword_identifier: (_) =>
      token(prec(21, new RegExp(`(?:${diagramTypes.join("|")})[A-Za-z0-9_]+`))),
